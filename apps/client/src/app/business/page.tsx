...
import { businessesApi, type Business as ApiBusiness } from "@/lib/api";
...
export default function BusinessPage() {
  ...
  const [businessList, setBusinessList] = useState<ApiBusiness[]>([]);
  ...
  const loadBusinesses = useCallback(async () => {
    ...
    try {
      const response = await businessesApi.listMine();
            ...
            >
              <form onSubmit={handleSubmit(handleBusinessCreation)} className="space-y-6 p-6 sm:p-7">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                  ...
                  <Button type="submit" disabled={isCreatingBusiness} className="bg-blue-600 text-white hover:bg-blue-700">
                    {isCreatingBusiness ? <Loader2 className="animate-spin" /> : "Create business"}
                  </Button>
                </div>
              </form>
            </motion.div>
