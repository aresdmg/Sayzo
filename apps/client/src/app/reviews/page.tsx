...
import { businessesApi, reviewsApi, type Business, type Review } from "@/lib/api";
...
export default function ReviewsPage() {
  ...
  useEffect(() => {
    ...
    businessesApi.listMine()
      .then((response) => {
        if (isMounted) setBusinessList(response.data ?? []);
      })
      .catch(() => {
        if (isMounted) setBusinessList([]);
      })
      .finally(() => {
        if (isMounted) setIsBusinessesLoading(false);
      });
  ...
  useEffect(() => {
    if (!effectiveSelectedBusinessId) {
      queueMicrotask(() => setReviewList([]));
      return;
    }
    ...
    queueMicrotask(() => {
      ...
      reviewsApi.getByBusinessId(effectiveSelectedBusinessId)
        .then((response) => {
          if (isMounted) setReviewList(response.data ?? []);
        })
        .catch(() => {
          if (isMounted) setReviewList([]);
        })
        .finally(() => {
